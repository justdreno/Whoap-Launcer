    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            if (loader === 'fabric' && result.instance?.id) {
                setCreateProgress('Installing Skin Loader...');
                try {
            ...
            if (selectedMods.length > 0 && result.instance?.id) {
                setCreateProgress(`Installing ${selectedMods.length} mods...`);
                for (let i = 0; i < selectedMods.length; i++) {
                    const mod = selectedMods[i];
                    setCreateProgress(`Installing ${mod.title} (${i + 1}/${selectedMods.length})...`);
                    try {
                        // Get compatible version for this mod
