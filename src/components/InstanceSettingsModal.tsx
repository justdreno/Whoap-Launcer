...
export const InstanceSettingsModal: React.FC<InstanceSettingsModalProps> = ({
    ...
    const handleShare = async () => {
        if (!user || user.type !== 'whoap') {
            showToast('Whoap account required to share.', 'error');
            return;
        }

        setLoadingFriends(true);
        const friendList = await CloudManager.getFriends(user.uuid);
    ...
    const confirmShare = async () => {
        ...
        const success = await CloudManager.shareInstance(user.uuid, selectedFriendId, {
            name: instance.name,
        ...
        if (success) {
            showToast('Instance shared successfully!', 'success');
            setSharing(false);
        } else {
            showToast('Failed to share instance.', 'error');
        }
    };
