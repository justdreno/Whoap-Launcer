...
// Component to load screenshot image via IPC
const ScreenshotImage: React.FC<{ screenshot: Screenshot; className?: string; alt?: string }> = ({ screenshot, className, alt }) => {
    ...
    if (loading) {
        return <div className={className} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#1a1a1a' }}>
...
export const Screenshots: React.FC<ScreenshotsProps> = ({ user }) => {
    ...
    const [shareScreenshot, setShareScreenshot] = useState<Screenshot | null>(null);
    ...
    const handleShare = (screenshot: Screenshot, event?: React.MouseEvent) => {
        event?.stopPropagation();
        ...
        <div className={styles.container}>
            ...
            {shareScreenshot && (
                <ShareScreenshotModal
                    ...
                    user={user}
                    onClose={() => setShareScreenshot(null)}
                />
            )}
        </div>
    );
